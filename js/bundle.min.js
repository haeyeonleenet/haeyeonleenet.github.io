...
`.replace(/id="photos" class="tab-pane.*?" style="display:.*?"/,`id="photos" class="tab-pane ${i==="photos"?"active":""}" style="display:${i==="photos"?"block":"none"};"`).replace(/id="video" class="tab-pane.*?" style="display:.*?"/,`id="video" class="tab-pane ${i==="video"?"active":""}" style="display:${i==="video"?"block":"none"};"`).replace(/href="\/media\/photos" class="tab-btn.*?"/,`href="/media/photos" class="tab-btn ${i==="photos"?"active":""}"`).replace(/href="\/media\/videos" class="tab-btn.*?"/,`href="/media/videos" class="tab-btn ${i==="video"?"active":""}"`),h=()=>`
    <section class="content-section fade-in">
        <h2>Schedule</h2>
        <ul class="schedule-list">
            ...
            <li class="schedule-item">
                ...
                <span class="event">Opera Debut: "La Boh\xE8me"</span>
...
`;var r=class{constructor(){this.routes={"/":c,"/about":d,"/media":m,"/schedule":h,"/contact":u},this.app=document.getElementById("app")}init(){let e=new URLSearchParams(window.location.search).get("p");e?(window.history.replaceState(null,null,e),this.handleRoute(e),this.updateActiveLink(e)):(this.handleRoute(window.location.pathname),this.updateActiveLink(window.location.pathname)),window.addEventListener("popstate",()=>{this.handleRoute(window.location.pathname),this.updateActiveLink(window.location.pathname)}),document.body.addEventListener("click",t=>{t.target.matches("[data-link]")&&(t.preventDefault(),this.navigateTo(t.target.href))})}navigateTo(a){window.history.pushState(null,null,a),this.handleRoute(window.location.pathname),this.updateActiveLink(window.location.pathname)}async handleRoute(a){let e=a.endsWith("/")&&a!=="/"?a.slice(0,-1):a;if(e==="/media"){this.navigateTo("/media/photos");return}let t,s=[];e==="/media/photos"?(t=this.routes["/media"],s=["photos"]):e==="/media/videos"?(t=this.routes["/media"],s=["video"]):t=this.routes[e]||this.routes["/"],!t&&!this.routes[e]&&(t=this.routes["/"]),this.app.innerHTML=t(...s),this.app.querySelectorAll(".tab-btn[data-tab]").forEach(b=>{b.addEventListener("click",l=>{let v=l.target.getAttribute("data-tab"),n=l.target.closest(".content-section");n.querySelectorAll(".tab-btn").forEach(o=>o.classList.remove("active")),l.target.classList.add("active"),n.querySelectorAll(".tab-pane").forEach(o=>{o.id===v?(o.style.display="block",o.classList.add("active")):(o.style.display="none",o.classList.remove("active"))})})});let p={"/":"Haeyeon Lee | Soprano","/about":"About | Haeyeon Lee","/media/photos":"Media (Photos) | Haeyeon Lee","/media/videos":"Media (Videos) | Haeyeon Lee","/schedule":"Schedule | Haeyeon Lee","/contact":"Contact | Haeyeon Lee"};document.title=p[e]||"Haeyeon Lee | Soprano",window.scrollTo(0,0)}updateActiveLink(a){let e=a.endsWith("/")&&a!=="/"?a.slice(0,-1):a;document.querySelectorAll(".site-nav a").forEach(t=>{let s=t.getAttribute("href");s===e||s==="/media"&&e.startsWith("/media/")?t.classList.add("active"):t.classList.remove("active")})}};document.addEventListener("DOMContentLoaded",()=>{document.getElementById("year").textContent=new Date().getFullYear(),new r().init();let a=document.querySelector(".mobile-nav-toggle"),e=document.querySelector(".site-nav");a&&e&&(a.addEventListener("click",()=>{e.classList.toggle("open")}),e.querySelectorAll("a").forEach(t=>{t.addEventListener("click",()=>{e.classList.remove("open")})}))});})();
