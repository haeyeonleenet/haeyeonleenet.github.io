...
`.replace(/id="photos" class="tab-pane.*?" style="display:.*?"/,`id="photos" class="tab-pane ${i==="photos"?"active":""}" style="display:${i==="photos"?"block":"none"};"`).replace(/id="video" class="tab-pane.*?" style="display:.*?"/,`id="video" class="tab-pane ${i==="video"?"active":""}" style="display:${i==="video"?"block":"none"};"`).replace(/href="\/media\/photos" class="tab-btn.*?"/,`href="/media/photos" class="tab-btn ${i==="photos"?"active":""}"`).replace(/href="\/media\/videos" class="tab-btn.*?"/,`href="/media/videos" class="tab-btn ${i==="video"?"active":""}"`),n=()=>`
    <section class="content-section fade-in">
        <h2>Schedule</h2>
...
`;var o=class{constructor(){this.routes={"/":s,"/about":r,"/media":l,"/schedule":n,"/contact":c},this.app=document.getElementById("app")}init(){let e=new URLSearchParams(window.location.search).get("p");e?(window.history.replaceState(null,null,e),this.handleRoute(e),this.updateActiveLink(e)):(this.handleRoute(window.location.pathname),this.updateActiveLink(window.location.pathname)),window.addEventListener("popstate",()=>{this.handleRoute(window.location.pathname),this.updateActiveLink(window.location.pathname)}),document.body.addEventListener("click",a=>{a.target.matches("[data-link]")&&(a.preventDefault(),this.navigateTo(a.target.href))})}navigateTo(t){window.history.pushState(null,null,t),this.handleRoute(window.location.pathname),this.updateActiveLink(window.location.pathname)}async handleRoute(t){if(t==="/media"||t==="/media/"){this.navigateTo("/media/photos");return}let e,a=[];t==="/media/photos"?(e=this.routes["/media"],a=["photos"]):t==="/media/videos"?(e=this.routes["/media"],a=["video"]):e=this.routes[t]||this.routes["/"],!e&&!this.routes[t]&&(e=this.routes["/"]),this.app.innerHTML=e(...a);let d={"/":"Haeyeon Lee | Soprano","/about":"About | Haeyeon Lee","/media/photos":"Media (Photos) | Haeyeon Lee","/media/videos":"Media (Videos) | Haeyeon Lee","/schedule":"Schedule | Haeyeon Lee","/contact":"Contact | Haeyeon Lee"};document.title=d[t]||"Haeyeon Lee | Soprano",window.scrollTo(0,0)}updateActiveLink(t){document.querySelectorAll(".site-nav a").forEach(e=>{let a=e.getAttribute("href");a===t||a==="/media"&&t.startsWith("/media/")?e.classList.add("active"):e.classList.remove("active")})}};document.addEventListener("DOMContentLoaded",()=>{document.getElementById("year").textContent=new Date().getFullYear(),new o().init();let t=document.querySelector(".mobile-nav-toggle"),e=document.querySelector(".site-nav");t&&e&&(t.addEventListener("click",()=>{e.classList.toggle("open")}),e.querySelectorAll("a").forEach(a=>{a.addEventListener("click",()=>{e.classList.remove("open")})}))});})();
