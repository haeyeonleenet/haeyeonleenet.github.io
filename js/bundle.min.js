...
`;var s=class{constructor(){this.routes={"/":c,"/about":d,"/media":m,"/schedule":h,"/contact":p},this.app=document.getElementById("app")}init(){let e=new URLSearchParams(window.location.search).get("p");e?(window.history.replaceState(null,null,e),this.handleRoute(e),this.updateActiveLink(e)):(this.handleRoute(window.location.pathname),this.updateActiveLink(window.location.pathname)),window.addEventListener("popstate",()=>{this.handleRoute(window.location.pathname),this.updateActiveLink(window.location.pathname)}),document.body.addEventListener("click",t=>{t.target.matches("[data-link]")&&(t.preventDefault(),this.navigateTo(t.target.href))})}navigateTo(i){window.history.pushState(null,null,i),this.handleRoute(window.location.pathname),this.updateActiveLink(window.location.pathname)}async handleRoute(i){let e=i.endsWith("/")&&i!=="/"?i.slice(0,-1):i;if(e==="/media"){this.navigateTo("/media/photos");return}let t,r=[];e==="/media/photos"?(t=this.routes["/media"],r=["photos"]):e==="/media/videos"?(t=this.routes["/media"],r=["video"]):t=this.routes[e]||this.routes["/"],!t&&!this.routes[e]&&(t=this.routes["/"]),this.app.innerHTML=t(...r),this.app.querySelectorAll(".tab-btn[data-tab]").forEach(v=>{v.addEventListener("click",l=>{let b=l.target.getAttribute("data-tab"),n=l.target.closest(".content-section");n.querySelectorAll(".tab-btn").forEach(o=>o.classList.remove("active")),l.target.classList.add("active"),n.querySelectorAll(".tab-pane").forEach(o=>{o.id===b?(o.style.display="block",o.classList.add("active")):(o.style.display="none",o.classList.remove("active"))})})});let u={"/":"Haeyeon Lee | Soprano","/about":"About | Haeyeon Lee","/media/photos":"Media (Photos) | Haeyeon Lee","/media/videos":"Media (Videos) | Haeyeon Lee","/schedule":"Schedule | Haeyeon Lee","/contact":"Contact | Haeyeon Lee"};document.title=u[e]||"Haeyeon Lee | Soprano",window.scrollTo(0,0)}updateActiveLink(i){let e=i.endsWith("/")&&i!=="/"?i.slice(0,-1):i;document.querySelectorAll(".site-nav a").forEach(t=>{let r=t.getAttribute("href");r===e||r==="/media"&&e.startsWith("/media/")?t.classList.add("active"):t.classList.remove("active")})}};document.addEventListener("DOMContentLoaded",()=>{document.getElementById("year").textContent=new Date().getFullYear(),new s().init();let i=document.querySelector(".mobile-nav-toggle"),e=document.querySelector(".site-nav");i&&e&&(i.addEventListener("click",()=>{e.classList.toggle("open")}),e.querySelectorAll("a").forEach(t=>{t.addEventListener("click",()=>{e.classList.remove("open")})}))});})();
